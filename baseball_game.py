...
class Solution(object):
    def calPoints(self, ops):
        ...
        last_valid = []
        operations = {'C', 'D', '+'}
        if ops[0] in operations:
            return False
        for i in range(len(ops)):
            if ops[i] == 'C':
                last_valid.pop()
            elif ops[i] == 'D':
                last_valid.append(2 * last_valid[len(last_valid) - 1])
            elif ops[i] == '+':
                last_valid.append(last_valid[len(last_valid) - 1] + last_valid[len(last_valid) - 2])
            else:
                last_valid.append(int(ops[i]))

        return sum(last_valid)


# Better solution, think about how to use python language
class Solution(object):
    def calPoints(self, ops):
        """
        :type ops: List[str]
