...
class Solution(object):
    def findDuplicate(self, paths):
        ...
        def return_name_content(current_file):
            for i in range(len(current_file)):
                if current_file[i] == '(':
                    return current_file[:i], current_file[i:(len(current_file) - 1)]
        ...
        for i in range(len(paths)):
            ...
            for j in range(1, len(current)):
                file_name, file_content = return_name_content(current[j])
                if file_content in map_file_index:
                    out[map_file_index[file_content]].append(current_path + '/' + file_name)
                else:
                    out.append([current_path + '/' + file_name])
                    map_file_index[file_content] = len(out) - 1
        i = 0
        while i < len(out):
            if len(out[i]) == 1:
                out.pop(i)
