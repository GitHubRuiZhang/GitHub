...
class Solution(object):
    def findRightInterval(self, intervals):
        ...
        int_list = sorted((e.start, i) for i, e in enumerate(intervals))
        out = []
        for i in range(len(intervals)):
            item = intervals[i]
            loc = binary_search(int_list, 0, len(intervals) - 1, item.end)
            if loc < len(intervals):
                out.append(int_list[loc][1])
            else:
                out.append(-1)
        return out
