...
class Solution(object):
    def removeComments(self, source):
        ...
        for it in source:
            i = 0
            while i < (len(it)):
                if left == False and it[i] == '/' and i + 1 < len(it) and it[i + 1] == '*':
                    left = True
                    i += 1
                elif left == True and it[i] == '*' and i + 1 < len(it) and it[i + 1] == '/':
                    left = False
                    i += 1
                elif left == False and it[i] == '/' and (i + 1 < len(it) and it[i + 1] == '/'):
                    break
                elif left == False:
                    cur += it[i]
                i += 1
            if left == False and cur != '':
                out.append(cur)
                cur = ''
            print(out, left)
        return out
