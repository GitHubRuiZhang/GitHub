...
class Solution(object):
    def areSentencesSimilarTwo(self, words1, words2, pairs):
        ...
        for i in range(len(pairs)):
            if pairs[i][0] in visited and pairs[i][1] in visited:
                par1 = dfs(pairs[i][0])
                par2 = dfs(pairs[i][1])
                visited[par2] = par1
            elif pairs[i][0] in visited:
                visited[pairs[i][1]] = dfs(pairs[i][0])
            elif pairs[i][1] in visited:
                visited[pairs[i][0]] = dfs(pairs[i][1])
            else:
                visited[pairs[i][1]] = pairs[i][0]
                visited[pairs[i][0]] = pairs[i][0]
        ...
        for i in range(len(words1)):
            if words1[i] == words2[i]:
                continue
            elif words1[i] in visited and words2[i] in visited and dfs(words1[i]) == dfs(words2[i]):
                continue
            else:
                return False
