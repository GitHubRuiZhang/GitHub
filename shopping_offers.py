...
class Solution(object):
    def verify(self, special, needs):
        for i in range(len(needs)):
            if needs[i] < 0 or special[i] > needs[i]:
                return False
        return True
    ...
    def shoppingOffers(self, price, special, needs):
        ...
        if sum(needs) == 0 or sum(price) == 0:
            return 0
        out = sum([needs[i] * price[i] for i in range(len(needs))])
        for s in range(len(special)):
            if special[s][:len(needs)] == needs:
                out = min(out, special[s][-1])
            else:
                count = 1
                new_needs = [it for it in needs]
                while self.verify(special[s], new_needs) == True:
                    new_needs = [new_needs[i] - special[s][i] for i in range(len(needs))]
                    # look for another special offers
                    cur = self.shoppingOffers(price, special[:s] + special[s + 1:], new_needs)
                    # directly buy the rest items
                    res = sum([new_needs[i] * price[i] for i in range(len(new_needs))])
                    out = min(out, cur + count * special[s][-1], res + count * special[s][-1])
                    count += 1
